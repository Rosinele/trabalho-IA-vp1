...
//Objetos e variáveis utilizada no projeto
    ...
    buscaProfundidade() {
        fetch(SERVER_URL + '/buscaProfundidade/buscando/item?objetivo='  +this.state.objetivo+ '&estado=' +this.state.estado)
            .then(r => r.json())
    ...
    buscaAmplitude() {
        fetch(SERVER_URL + '/buscaAmplitude/buscando/item?objetivo=' +this.state.objetivo+ '&estado=' +this.state.estado)
            .then(r => r.json())
    ...
    render() {
        return (
            <div class="todo">
                <label class="top">Informe o estado atual:
                    <input type="text" name="estado"  class="etop" id="estado" maxlength="9" value={this.state.value} onChange={this.handleChangeEstado} /><br/>
                </label>
                <br/>
                <label class="top">Informe o objetivo:
                    <input type="text" name="objetivo" class="etop"  id="objetivo" maxlength="9" value={this.state.value} onChange={this.handleChangeObjetivo} /><br/>
                </label>
                <br/><br/>
                {/* Chama o método*/}
